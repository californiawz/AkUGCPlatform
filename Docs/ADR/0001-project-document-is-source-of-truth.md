...
- 当前支持按 V0 → V1 → V2 → V3 连续迁移：V1 补齐项目版本并将缺失或为零的 Component SchemaVersion 规范化为 1；V2 为旧场景补齐空 Logic Graph；V3 引入 Timer 与 Spawn 节点参数。
- Logic Graph 进入持久化契约后至少使用 V2，Timer 与 Spawn 使用 V3；旧文档加载时自动迁移，避免旧客户端把逻辑字段当作未知数据丢弃。
