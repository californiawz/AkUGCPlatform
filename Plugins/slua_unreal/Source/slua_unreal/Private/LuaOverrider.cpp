...
namespace NS_SLUA
{
    ...
    {
        // Shutdown 时序下 cls 可能已被回收（悬垂指针），
        // IsValid 内部 IsValidLowLevel 通过 vtable 访问已释放内存，
        // 会安全返回 false 而非崩溃或触发引擎警告。
        if (!IsValid(cls))
            return;
#if WITH_EDITOR
        if (!bObjectDeleted)
    ...
    {
        // Shutdown 时 cls 可能已被回收（悬垂指针），IsValid 内部
        // IsValidLowLevel 通过 vtable 访问已释放内存但不会崩溃，仅返回 false。
        if (!IsValid(cls))
        {
            return;
        }
